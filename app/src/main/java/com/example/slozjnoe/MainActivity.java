...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        aapi = retrofit.create(api.class);
        ...
        locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 1000, 100, new LocationListener() {
            ...
            public void onLocationChanged(@NonNull Location location) {
                ...
                aapi.getWeather(location.getLatitude(),location.getLongitude(), "e012f24a2044e4fe9504ac4db488f059", "metric").enqueue(new Callback<Weather>() {
                    @Override
                    public void onResponse(Call<Weather> call, Response<Weather> response) {
                        if (response.code() == 200){
                            Weather weather = response.body();
                            temp.setText("Температура: "+ weather.getCurrent().getTemp() + "\nОщущается как: "+weather.getCurrent().getFeels_like());
